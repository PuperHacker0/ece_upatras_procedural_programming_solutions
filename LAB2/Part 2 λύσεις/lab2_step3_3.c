...
double next_temps(double matrix[N][M]);
void normalize(double matrix[N][M], short normalized_matrix[N][M]);
void print_histogram(short matrix[N][M], int x);

int main() {
    int x;
    printf("Which second is of interest to you? ");
    scanf("%d", &x);
    ...
    int time_interval = 0; //Seconds
    double delta;
    while (1) {
        normalize(matrix, normalized_matrix);

        if (time_interval == x)
            print_histogram(normalized_matrix, x);

        if ((delta = next_temps(matrix)) < 1.0)
            break; //End condition
        ++time_interval; //Step of 1 second each time
    }
    
    //Special case: if the number x that is given
    //Is after the end condition
    if (x > time_interval)
        print_histogram(normalized_matrix, x);

    //Print end condition temperatures
    print_matrix(matrix, time_interval);
...
void print_matrix(double matrix[N][M], int time) {
    printf("The matrix at the end condition (t=%d) is:\n", time);
...
double next_temps(double matrix[N][M]) {
    double prev_matrix[N][M], delta = 0;
    ...
    for (int r = 1; r < N - 1; ++r)
        for (int c = 1; c < M - 1; ++c) {
            ...
            delta += fabs(matrix[r][c] - prev_matrix[r][c]);
        }
...
void print_histogram(short normalized_matrix[N][M], int x) {
    int histogram[10] = {};
    for (int r = 0; r < N; ++r)
        for (int c = 0; c < M; ++c)
            ++histogram[normalized_matrix[r][c]];

    printf("\n\nHistogram at time interval x=%d:\n", x);
    for (int i = 0; i <= 9; ++i)
        printf("Rank %d -> %d cells\n", i, histogram[i]);
